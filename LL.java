...
public class LL {
    ...
    public static double[] getAbsolutePixelFromLonLat(double lon, double lat, int zoom, int delx, int dely) {
        double pixelX = (lon + 180) / 360 * Math.pow(2, zoom) * 256 - delx * 256;
        double pixelY = (1 - Math.log(Math.tan(lat * Math.PI / 180) + 1 / Math.cos(lat * Math.PI / 180)) / Math.PI) / 2 * Math.pow(2, zoom) * 256 - dely * 256;
    ...
    public static List<double[]>   dantiaoxianlu(double[][] pqx) {
        double k=pqx[0][0];
        ...
        for (int i = 0; i < pqx.length; i++) {
            ...
            while (k <= dtk + dtcd) {
                double hudu = dmsToRadians(dtfwj); // 假设已存在此方法
                double [] xy= zs(dtk, dtx, dty, hudu, dtcd, dtr1, dtr2, dtzy, k, 0, 90); 
                xys.add(xy);
                k = k + 20;
            }
        }
    ...
    public static double[] mapGps2Xy(double lon, double lat, String proj, double[][] lonlat, double[][] xy, double center) {
        if (proj.contains("gao")) {
            double[] sourceXy = new double[lonlat.length * 2];
            double[] targetXy = new double[lonlat.length * 2];
            for (int i = 0; i < lonlat.length; i++) {
                double[] ls = gauss_proj(lonlat[i][0], lonlat[i][1], center);
                sourceXy[i * 2] = ls[0];
                sourceXy[i * 2 + 1] = ls[1];
                targetXy[i * 2] = xy[i][0];
                targetXy[i * 2 + 1] = xy[i][1];
            }
            double[] cs4 =cs4(sourceXy, targetXy);
            double[] lsxy = gauss_proj(lon, lat, center);
            return fourParameterTransform(lsxy[0], lsxy[1], cs4[0], cs4[1], cs4[2], cs4[3]);
        }
        if (proj.contains("utm")) {
            double[] sourceXy = new double[lonlat.length * 2];
            double[] targetXy = new double[lonlat.length * 2];
            for (int i = 0; i < lonlat.length; i++) {
                double[] ls = utm_proj(lonlat[i][0], lonlat[i][1]);
                sourceXy[i * 2] = ls[0];
                sourceXy[i * 2 + 1] = ls[1];
                targetXy[i * 2] = xy[i][0];
                targetXy[i * 2 + 1] = xy[i][1];
    ...
    public static double[] mapXy2Gps(
        double x,
        double y,
        String proj,
        double[][] lonlat,
        ...
        if (lonlat == null || xy == null || lonlat.length == 0 || xy.length == 0 || lonlat.length != xy.length) {
        ...
        // 计算平均经度作为中心经度（用于高斯投影）
        double center = 0.0;
        for (double[] ll : lonlat) {
            center += ll[0]; // 经度
        }
        center = center / lonlat.length;

        if (proj.toLowerCase().contains("gao")) { // 高斯投影
            for (int i = 0; i < lonlat.length; i++) {
                double lon = lonlat[i][0];
                double lat = lonlat[i][1];
                double[] projected = gauss_proj(lon, lat, center);
            ...
            double[] cs4 = cs4(targetXY, sourceXY);
            double[] lsxy = fourParameterTransform(x, y, cs4[0], cs4[1], cs4[2], cs4[3]);

            return gauss_unproj(lsxy[0], lsxy[1], center);

        } else if (proj.toLowerCase().contains("utm")) { // UTM 投影
            boolean north = true;
            if (lonlat[0][1] < 0) { // 纬度小于0为南半球
                north = false;
            }
            ...
            for (int i = 0; i < lonlat.length; i++) {
                double lon = lonlat[i][0];
                double lat = lonlat[i][1];
                double[] projected = utm_proj(lon, lat);
            ...
            double[] unprojResult = utm_unproj(lsxy[0], lsxy[1], north, zone);

            // 假设 Utm_unproj 返回的是 [经度, 纬度]，如果还返回了其他信息，按需调整
            if (unprojResult != null && unprojResult.length >= 2) {
                return new double[]{unprojResult[0], unprojResult[1]};
            } else {
                return new double[]{0, 0};
            }
